...
from app.scoring import score_events, get_mvp
...
def detect_turning_points(events):
    ...
    return sorted([turn for turn, score in turn_scores.items() if score >= threshold])

def analyze_replay(log_text: str, format_id: str | None = None, winner: str | None = None) -> BattleAnalysis:
    ...
    threats = sorted(
        [p for p in impacts.values() if p.score >= 4],
